...
Vue.createApp({
  ...
  methods: {
    ...
    setbonusetype(index){
      if(confirm("Register effects pattern, OK?")){
        let ans = []
        for(let i=0;i<15;i++){
    ...
    setrankbonusetype(index){
      if(confirm("Register greater effects pattern, OK?")){
        let ans = []
        for(let i=0;i<15;i++){
    ...
    setmodetype(){
      if(confirm('Register mode patern, OK?')){
        for(let i=0;i<8;i++){
    ...
    resetData(force) {
      if (force || confirm('This is HARD RESET. EVERYTHING will be LOST. REALLY REALLY REALLY OK?')) {
    ...
    resetDarklevel(){
      ...
      if(confirm('prestige darklevel for ' + gaindarklevel + ' ?')){
    ...
    resetLevel(force,exit) {
      if(this.player.onchallenge && this.player.challenges.includes(0)){
        if(this.player.money.lt(new Decimal('1e24'))){
          alert('Now on challenge condition 1, cannot do level reset.')
          return;
        }
      }
    ...
    resetRank(force){
      ...
      if(this.player.onchallenge && this.player.challenges.includes(0)){
        if(this.player.money.lt(this.resetRankborder())){
          alert('Now on challenge condition 1, cannot do rank reset.')
          return;
        }
      }
    ...
    startChallenge(){
      ...
      if(challengeid == 0){
        alert("No challenge conditions choosed.")
        return;
      }

      let conf = 'Start challenge? Points, generators, accelerators will be lost.'

      if(this.player.challengecleared.includes(challengeid)){
        if(this.player.challengecleared.length<128){
          alert("Completed challenge.")
          return;
        }
        conf = 'Completed challenge. No token, OK?'
        if(this.player.rankchallengecleared.includes(challengeid)){
          conf = 'Completed rank challenge. No greater token, OK?'
        }
      }
    ...
    exitChallenge(){
      if (confirm('Give up challenge? you will be in normal condition. points, generetors, etc... will not be lost.')) {
        this.player.onchallenge = false;
        this.activechallengebonuses = this.player.challengebonuses;
    ...
    shrinkworld(i){
      if(4>this.trophynumber[i]){
        alert("Cannot shrink worlds with less than or equal to 4 milestones.")
        return
      }
      if(this.players[i].remember>=this.trophynumber[i]){
        alert("milestones are less than remembers.")
        return
      }
      if(confirm("shrink world "+(i+1)+" and gain remembers? Nothing will remain in shrinked world and you will restart the world.")){
